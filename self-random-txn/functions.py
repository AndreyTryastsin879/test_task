...
def create_report(report_table, chain_name, private_key, transaction_type, amount, trasaction_url, error):
    d={}
...
def self_transaction(web3, amount, private_key,
    ...
    print(f'SELF-транзакция {chain_name}')
    ...
    create_report(report_table, chain_name, 
                  private_key, 'SELF', 
                  amount,
...
def random_transaction(web3, amount, private_key,
    ...
    print(f'RANDOM-транзакция {chain_name}')
...
def main_function(private_key):
    ...
    for chain in map(chain_settings.__dict__.get, chain_settings.__all__): 
        try:
            ...
            web3 = create_connection(chain['url'], chain['port'])

            if web3.is_connected() == False:
                print(f'Соединение не установлено', '\n')
                report_table = open_csv()
                create_report(report_table, chain_name, '-', '-', '-', '-', 'Соединение не установлено')   
